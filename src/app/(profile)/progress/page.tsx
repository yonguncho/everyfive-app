...
export default function ProgressPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [profileRes, dailyStatsRes, wordStateRes, upcomingRes, badgesRes, dueNowRes, subRes] = await Promise.allSettled([
      ...
      setIsFree(!(subData && activeStatuses.includes(subData.status)));
  ...
  return (
    <div className="space-y-4 py-6">
      ...
      {isFree && (
        <a
          href="/subscription"
          className="flex items-center gap-4 rounded-2xl border border-brand/20 bg-gradient-to-r from-indigo-50 to-purple-50 px-4 py-4"
        >
          <div className="flex-1">
            <div className="text-sm font-semibold text-gray-800">Pro로 업그레이드하면</div>
            <div className="text-xs text-gray-500 mt-0.5">하루 10~20단어 → 그래프가 3~4배 빠르게 채워집니다</div>
          </div>
          <div className="text-right flex-shrink-0">
            <div className="text-xs font-bold text-brand">업그레이드 →</div>
            <div className="text-xs text-gray-400 mt-0.5">7일 무료</div>
          </div>
        </a>
      )}
