...
export default function DailyPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const alreadyDone = localStorage.getItem('ef_daily_done') === todayKst;
      ...
      // 오늘 세션 이미 완료 → 완료 화면 바로 표시
      if (alreadyDone) {
        const savedCount = parseInt(localStorage.getItem('ef_daily_count') ?? '5');
        const [dueResult, subResult] = await Promise.all([
          supabase
            .from('user_word_state')
        ...
        setIsFree(!(sub && activeStatuses.includes(sub.status)));
        setWords(Array(savedCount).fill(SAMPLE_WORDS[0]));
        setDone(true);
        setLoading(false);
        return;
      }
  ...
  async function onWordComplete(result: { recognitionResult: 'success'|'failed'|'skipped'; quizCorrect: boolean | null; speechScore?: number }) {
    ...
    if (idx + 1 >= words.length) {
      ...
      // 오늘 완료 상태 저장 (재진입 시 중복 세션 방지)
      const todayKst = new Date(Date.now() + 9 * 60 * 60 * 1000).toISOString().slice(0, 10);
      localStorage.setItem('ef_daily_done', todayKst);
      localStorage.setItem('ef_daily_count', String(words.length));
      setDone(true);
    } else {
      setIdx(idx + 1);
    }
    ...
    return (
      <div className="space-y-6 py-12 text-center">
        ...
        {isFree && (
          <a
            href="/subscription"
            className="flex items-center gap-3 rounded-2xl bg-brand px-5 py-4 text-white text-left shadow-sm"
          >
            <span className="text-2xl">⭐</span>
            <div className="flex-1">
              <div className="font-semibold text-sm">하루 10~20단어로 늘리기</div>
              <div className="text-xs text-white/80 mt-0.5">Pro 업그레이드 · 7일 무료 체험</div>
            </div>
  ...
  return (
    <div className="py-4">
      ...
      {/* 마지막 단어 + 무료 유저 → 업그레이드 넛지 */}
      {isFree && idx === words.length - 1 && (
        <div className="mb-3 flex items-center justify-between rounded-xl bg-amber-50 border border-amber-100 px-4 py-2.5">
          <span className="text-sm text-amber-700 font-medium">오늘의 마지막 단어예요</span>
          <a href="/subscription" className="text-xs font-bold text-brand whitespace-nowrap">
            Pro면 15단어 더 →
          </a>
        </div>
      )}
