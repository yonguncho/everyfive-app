-- v1.6: pg_cron으로 send-reminder Edge Function 매일 UTC 00:00 (KST 09:00) 호출
--
-- 사전 조건:
--   1. app.cron_secret GUC 설정 (Supabase Dashboard > SQL Editor):
--      ALTER DATABASE postgres SET "app.cron_secret" = '<CRON_SECRET값>';
--      SELECT pg_reload_conf();
--   2. Edge Function send-reminder에 CRON_SECRET secret 설정 (같은 값):
...
-- 기존 스케줄 제거 (없으면 무시)
DO $$ BEGIN
  PERFORM cron.unschedule('send-reminder');
...
SELECT cron.schedule(
  ...
  $$
    SELECT net.http_post(
      url     := 'https://qftncrkvsergtyhilzwa.supabase.co/functions/v1/send-reminder',
